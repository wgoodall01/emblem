...
if (fs.existsSync(DATA_CACHE)) {
  ...
  fs.writeFileSync(
    DATA_CACHE,
    JSON.stringify({ postList, users, userKeys }, null, 4)
  );
...
const submitPost = post =>
  rp({ uri: `${URI}/post`, method: "POST", body: post, json: true });
const registerUser = user =>
  rp({ uri: `${URI}/register`, method: "POST", body: user, json: true });

const postReqs = postList.map(submitPost);
const registerReqs = Object.values(users).map(registerUser);

const allReqs = Promise.all([Promise.all(postReqs), Promise.all(registerReqs)]);
